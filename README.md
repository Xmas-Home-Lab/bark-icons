...
PNG 文件为 `512 × 512`、sRGB、透明画布。Telegram 和 Bitcoin 等圆形图标保留满幅圆形；黄金和天气图标裁掉原始透明留白后，将最长边统一为约 82%，并居中保留安全边距。文件名带版本号，因为 Bark 会永久缓存相同 URL 的图标。
...
天气图标来自 [Meteocons](https://github.com/basmilius/meteocons) 的 `@meteocons/svg-static` Fill `0.1.0`，依据 [MIT License](https://github.com/basmilius/meteocons/blob/main/LICENSE) 使用；本仓库仅进行 PNG 转换、裁边、缩放和居中。
